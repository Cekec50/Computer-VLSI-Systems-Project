...
class scoreboard extends uvm_scoreboard;
	...
	virtual function write(register_item item);
		...
		else if (item.inc) register = register + 4'h01;
		else if (item.dec) register = register - 4'h01;
		else if (item.sr) register = (register >> 1) | {item.ir, 3'b000};
		else if (item.sl) register = (register << 1) | {3'b000, item.il};
	endfunction
